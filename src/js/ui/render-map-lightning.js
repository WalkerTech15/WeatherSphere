...
function statusHtml(status, errorKind) {
  if (status === "loading") return `<p class="map-lightning-status" data-loading="1">${esc(t("mapLightningLoading"))}</p>`;
  if (status === "empty") return `<p class="map-lightning-status" data-state="clear">${esc(t("mapLightningNone"))}</p>`;
  if (status === "unsupported") return `<p class="map-lightning-status" data-state="unsupported">${esc(t("mapLightningUnavailable"))}</p>`;
...
function readyHtml(data) {
  const rows = data.strikes.slice(0, 6).map((strike) => `
    <li class="map-lightning-strike"><span>${esc(strike.type || t("mapLightningStrike"))}</span><strong>${Number.isFinite(strike.amperage) ? `${Math.round(strike.amperage)} A` : "—"}</strong></li>`).join("");
...
export function lightningAnnouncement(state) {
  ...
  if (state.status === "ready") return t("mapLightningRecent").replace("{n}", String(state.data.strikes.length));
...
export function renderLightningUI(lightningState) {
  ...
  host.hidden = false;
  host.innerHTML = lightningState.status === "ready"
