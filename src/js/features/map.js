...
function resetOverlay(type) {
  ...
  /* leaving Air Quality (for any other layer, including satellite) drops
     its reading — re-selecting it later starts a fresh loading state
     rather than flashing the previous place's numbers for a frame */
  if (type !== "airQuality") {
...
export async function setMapLayer(type, { offset = overlay.offset } = {}) {
  /* Air Quality has no MapTiler weather layer (WEATHER_LAYER_IDS
     deliberately excludes it — see weather-layers.js) and follows its own
     point-reading flow instead of the ramp-layer one below. */
