...
function renderWeatherOverlay() {
  ...
  if (overlay.type === "airQuality") renderAirQualityUI(airQualityState);
  if (overlay.type === "humidity") {
...
function resetOverlay(type) {
  ...
  /* leaving Air Quality or Humidity (for any other layer, including
     satellite) drops its reading — re-selecting it later starts a fresh
     loading state rather than flashing the previous place's numbers for a
     frame */
  if (type !== "airQuality") {
  ...
  /* leaving Alerts drops both the reading and anything drawn for it, so a
     previous place's warning area can never linger over a new place */
  if (type !== "alerts") {
...
export async function setMapLayer(type, { offset = overlay.offset } = {}) {
  /* Air Quality, Humidity and Alerts have no MapTiler weather layer
     (WEATHER_LAYER_IDS deliberately excludes all three — see
     weather-layers.js) and each follows its own point-reading flow instead
     of the ramp-layer one below. */
...
/* Fetches and renders the Air Quality reading for `loc`. Shared by
   ...
   resolving late. */
async function loadAirQualityFor(loc, requestId, isStale, button) {
  aqiController?.abort();
...
/* Selecting the Air Quality layer: no spatial layer exists for it, so the
   ...
   the reading did. */
async function setAirQualityLayer() {
  const requestId = ++layerRequestId;
  const isStale = () => requestId !== layerRequestId;
  ...
  await loadAirQualityFor(state.loc, requestId, isStale, button);
}
...
async function setHumidityLayer() {
  const requestId = ++layerRequestId;
  const isStale = () => requestId !== layerRequestId;
...
async function loadAlertsFor(loc, requestId, isStale, button) {
...
async function setAlertsLayer() {
  ...
  await loadAlertsFor(state.loc, requestId, isStale, button);
}
